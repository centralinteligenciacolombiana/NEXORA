...
import {
  AdminMovesClient,
  type AdminMoveRow,
} from "@/components/admin/admin-moves-client";
...
export default async function AdminMovesPage() {
  ...
  const [{ data: rows }, { data: units }] = await Promise.all([
    supabase
      .from("move_requests")
      .select(
        "id, request_type, proposed_at, moving_company, notes, status, review_notes, unit_id, requested_by, verified_at, verified_by",
      )
  ...
  const list: AdminMoveRow[] = (rows ?? []).map((r) => ({
    id: r.id,
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--muted)]">
            Autoriza ingresos/salidas y revisa la verificación de portería.
          </p>
        </div>
      </div>

      <AdminMovesClient rows={list} />
    </div>
  );
}
