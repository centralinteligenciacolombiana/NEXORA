-- =============================================================================
-- NEXORA — 06_invite_register_units.sql
-- Unidades públicas vía token de invitación válido + accept_invite con unit_id
-- =============================================================================

CREATE OR REPLACE FUNCTION public.get_units_for_invite(p_token text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF (v_payload ->> 'valid')::boolean IS NOT TRUE THEN
    RETURN jsonb_build_object(
      'valid', false,
      'error', COALESCE(v_payload ->> 'error', 'Invitación inválida'),
      'units', '[]'::jsonb
    );
  END IF;

  v_complex_id := (v_payload -> 'complex' ->> 'id')::uuid;
  ...
  SELECT COALESCE(
    jsonb_agg(
      ...
      ORDER BY u.tower NULLS LAST, u.number
    ),
    '[]'::jsonb
  )
  INTO v_units
  FROM public.units u
  WHERE u.complex_id = v_complex_id
    AND u.is_active = true;

  RETURN jsonb_build_object(
    'valid', true,
    'complex_id', v_complex_id,
    'units', v_units
  );
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_units_for_invite(text) TO anon, authenticated;

-- Extender accept_invite para aceptar unit_id directamente
...
BEGIN
  ...
  IF v_unit_id IS NOT NULL THEN
    ...
    END IF;
  ELSIF p_unit_number IS NOT NULL AND length(trim(p_unit_number)) > 0 THEN
...
GRANT EXECUTE ON FUNCTION public.accept_invite(text, text, text, uuid) TO authenticated;
